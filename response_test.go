...
func TestResponseCases(t *testing.T) {
	cases := []struct {
		...
		},
		{
			name: "options",
			build: func() Response {
				return NewResponse(WithBody([]byte("hello")), WithHeader("X-Test", "abc"), WithStatusCode(http.StatusAccepted))
			},
			expectBody:    "hello",
		...
		},
		{
			...
			build:         func() Response { return NewStatusResponse(http.StatusAccepted, WithHeader("X-Test", "abc")) },
			expectBody:    "",
			expectStatus:  http.StatusAccepted,
			expectHeaders: map[string]string{"X-Test": "abc"},
		},
		...
		},
		{
			...
			build: func() Response {
				return NewTextResponse("plain", WithStatusCode(http.StatusCreated), WithHeader("X-Test", "abc"))
			},
			expectBody:    "plain",
		...
		},
		{
			...
			build: func() Response {
				return NewJsonResponse(sample{Name: "bob", Age: 21}, WithStatusCode(http.StatusCreated), WithHeader("X-App", "test"))
			},
