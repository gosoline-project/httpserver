...
func init() {
	suite.RegisterTestCaseDefinition("gosoline-httpserverExtended", isTestCaseHttpserverExtended, buildTestCaseHttpserverExtended)
}
...
func isTestCaseHttpserverExtended(_ suite.TestingSuite, method reflect.Method) error {
...
func buildTestCaseHttpserverExtended(suite suite.TestingSuite, method reflect.Method) (suite.TestCaseRunner, error) {
...
func runHttpServerExtendedTestsMap(s suite.TestingSuite, testCases map[string]ToHttpserverTestCaseList) (suite.TestCaseRunner, error) {
	testCaseRunners := make([]suite.TestCaseRunner, 0, len(testCases))

	for name, testCasesProvider := range testCases {
		runner, err := runHttpServerExtendedTests(s, func() []*HttpserverTestCase {
		...
		testCaseRunners = append(testCaseRunners, func(t *testing.T, suite suite.TestingSuite, suiteConf *suite.SuiteConfiguration, environment *env.Environment) {
	...
	return func(t *testing.T, s suite.TestingSuite, suiteConf *suite.SuiteConfiguration, environment *env.Environment) {
		...
		for _, testCaseRunner := range testCaseRunners {
			testCaseRunner(t, s, suiteConf, environment)
		}
	}, nil
}

func runHttpServerExtendedTests(s suite.TestingSuite, getTestCases func() []*HttpserverTestCase) (suite.TestCaseRunner, error) {
	return runTestCaseHttpserver(s, func(s suite.TestingSuite, app suite.AppUnderTest, client *resty.Client) {
		suite.RunTestCaseInSuite(s.T(), s, func() {
			...
			responses := runHttpServerExtendedRequests(s, testCases, client)

			app.Stop()
			app.WaitDone()

			verifyHttpServerExtendedResponses(s, testCases, responses)
		})
	})
}

func runHttpServerExtendedRequests(suite suite.TestingSuite, testCases []*HttpserverTestCase, client *resty.Client) []*resty.Response {
...
func verifyHttpServerExtendedResponses(suite suite.TestingSuite, testCases []*HttpserverTestCase, responses []*resty.Response) {
	for i, tc := range testCases {
		if tc.Assert != nil {
