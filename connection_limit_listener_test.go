...
func TestConnectionLimitListenerTestSuite(t *testing.T) {
...
func (s *ConnectionLimitListenerTestSuite) TestAcceptWaitsUntilConnectionSlotIsReleased() {
	...
	go func() {
		...
		require.NoError(s.T(), acceptErr)
		require.NotNil(s.T(), connB)
		acceptedSecond.Store(true)
	}()

	s.Never(acceptedSecond.Load, 20*time.Millisecond, time.Millisecond)

	s.Require().NoError(limitedConnA.Close())
...
func (s *ConnectionLimitListenerTestSuite) TestCloseUnblocksWaitingAccept() {
	...
	s.Require().NoError(limited.Close())
	s.Require().Eventually(acceptReturned.Load, time.Second, time.Millisecond)
