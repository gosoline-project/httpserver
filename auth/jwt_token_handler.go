...
//go:generate go run github.com/vektra/mockery/v2 --name JwtTokenHandler --with-expecter
...
func NewJwtTokenHandler(config cfg.Config, name string) (JwtTokenHandler, error) {
	key := fmt.Sprintf("%s.jwt", configAuthKey(name))
	settings := &JwtTokenHandlerSettings{}
...
func (h *jwtTokenHandler) SignClaims(claims Claims) (*string, error) {
	...
	if tokenString, err = token.SignedString([]byte(h.settings.SigningSecret)); err != nil {
