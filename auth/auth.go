...
//go:generate go run github.com/vektra/mockery/v2 --name Authenticator --with-expecter
...
func configAuthKey(name string) string {
	return fmt.Sprintf("httpserver.%s.auth", name)
}
