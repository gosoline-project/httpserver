...
func NewTokenBearerHandler(config cfg.Config, logger log.Logger, name string, provider TokenBearerProvider) (gin.HandlerFunc, error) {
	...
	if auth, err = NewTokenBearerAuthenticator(config, logger, name, provider); err != nil {
...
func NewTokenBearerAuthenticator(config cfg.Config, logger log.Logger, name string, provider TokenBearerProvider) (Authenticator, error) {
	var err error
	var keyHeader string
	var tokenHeader string
	key := fmt.Sprintf("%s.bearer", configAuthKey(name))

	if keyHeader, err = config.GetString(fmt.Sprintf("%s.id_header", key)); err != nil {
		return nil, err
	}

	if tokenHeader, err = config.GetString(fmt.Sprintf("%s.token_header", key)); err != nil {
		return nil, err
	}
...
func (a *tokenBearerAuthenticator) IsValid(ginCtx *gin.Context) (bool, error) {
	...
	if bearer, err = a.provider(ginCtx.Request.Context(), bearerId, token); err != nil {
