...
func NewConfigKeyHandler(config cfg.Config, logger log.Logger, name string, provider ApiKeyProvider) (gin.HandlerFunc, error) {
	...
	if auth, err = NewConfigKeyAuthenticator(config, logger, name, provider); err != nil {
...
func NewConfigKeyAuthenticator(config cfg.Config, logger log.Logger, name string, provider ApiKeyProvider) (Authenticator, error) {
	var err error
	var keys []string

	if keys, err = config.GetStringSlice(fmt.Sprintf("%s.keys", configAuthKey(name))); err != nil {
